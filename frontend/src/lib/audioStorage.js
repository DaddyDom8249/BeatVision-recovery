...
function fallbackKey(projectId) {
  return `${FALLBACK_KEY_PREFIX}${projectId}`;
}
...
export async function getStoredAudioFile(projectId) {
  ...
  try {
    ...
    // Fall through to the small-file localStorage fallback.
  }
...
export async function removeStoredAudioFile(projectId) {
  ...
  try {
    ...
    // Missing IndexedDB is harmless when there is no stored record there.
  }
}
